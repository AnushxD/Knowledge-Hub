...
--
-- Applies the schema to a fresh (or partially migrated) Postgres database and
-- seeds the default administrator. Safe to run repeatedly: each migration is
-- guarded by a check against __ef_migrations_history, and the admin insert is
-- guarded by ON CONFLICT, so re-running this script only ever applies what is
-- actually missing.
--
-- The chain here starts at `InitialSchemaV2`, which is a squash and not a step
-- on from release 1. A database still carrying release 1's migration history
-- must not be pointed at this script: the guards would find none of those ids
-- recorded and try to create tables that already exist under a different
-- shape. Recreate such a database instead. Release 1's own frozen script is on
-- the `release-1` branch.
--
-- Create the database first — the API never does, in keeping with this
-- project's rule that provisioning is explicit:
--
--   createdb -U documenthub documenthub
--
...
-- file.
--
--   perl -i -pe 's/^\x{ef}\x{bb}\x{bf}//' documenthub-schema.sql
